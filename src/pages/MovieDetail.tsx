...
export default function MovieDetail() {
  ...
  useEffect(() => {
    if (movie && movieId !== prevMovieRef.current) {
      addToHistory(movie);
      prevMovieRef.current = movieId;
    }
  }, [movie, movieId, addToHistory]);
          ...
          <div className="flex-1 text-white">
            ...
            <div className="flex flex-wrap gap-3 mt-7">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-zinc-800 border border-zinc-600 text-white font-bold px-4 py-3 rounded-md hover:bg-zinc-700 transition disabled:opacity-60 disabled:cursor-not-allowed"
                title="Manual download with custom URL"
              >
                <Download size={18} />
              </button>
        ...
        <div className="fixed inset-0 bg-black z-[100] flex flex-col">
            ...
            {mode === "embed" ? (
              ...
              <CustomPlayer
                ...
                onBookmark={(time, note) => {
                  console.log(`Bookmark at ${time}: ${note}`);
                }}
              />
            ) : (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 overflow-y-auto">
                <div className="max-w-2xl w-full">
                  ...
                  <p className="text-zinc-500 text-xs mt-5">
                    💡 {subtitles.length > 0
                      ? `${subtitles.length} subtitle tracks found.`
          ...
          {mode === "embed" && (
            <div className="bg-black border-t border-zinc-900 p-3 flex flex-wrap items-center gap-2 justify-center">
              <span className="text-zinc-500 text-xs mr-2">Sources:</span>
      ...
      {showDownload && (
        ...
        >
          <div className="bg-zinc-900 rounded-2xl max-w-lg w-full p-6 border border-zinc-800" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-zinc-400 text-xs mb-2">Demo streams:</p>
            <div className="space-y-1.5">
              {[
                { label: "Sintel (~130 MB)", url: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4" },
                { label: "Tears of Steel (~170 MB)", url: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/TearsOfSteel.mp4" },
                { label: "Big Buck Bunny (~150 MB)", url: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4" },
              ].map((d) => (
                <button
                  key={d.url}
            ...
            <p className="text-zinc-500 text-xs mt-4 text-center">
              View progress from <Link to="/downloads" className="text-[#e50914] underline">My Library</Link>
            </p>
          </div>
        </div>
      )}
