...
function MovieCardImpl({ movie, large = false }: { movie: Movie; large?: boolean }) {
  ...
  // Check if it's a TV show - MORE RELIABLE DETECTION
  ...
  // IMPORTANT: Use /tv/ for TV shows, /movie/ for movies
  const detailUrl = isTv ? `/tv/${movie.id}` : `/movie/${movie.id}`;
    ...
    >
      <Link to={detailUrl}>
        ...
        {poster ? (
          ...
          <div className={`w-full ${large ? "h-64 md:h-72" : "h-48 md:h-56"} flex items-center justify-center text-zinc-500 text-xs p-2 text-center bg-zinc-800`}>
            {title}
          </div>
        )}
        ...
        {/* Watched badge */}
        {progressPercent >= 100 && (
          <div className="absolute bottom-12 left-2 bg-green-500/90 text-white text-[9px] font-bold px-2 py-0.5 rounded-full">
            ✓ Watched
          </div>
        )}
      </Link>
